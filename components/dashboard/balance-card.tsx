...
import { ChainDropdownMobile } from './chain-dropdown-mobile';
...
export function BalanceCard() {
  ...
  return (
    <div className="bg-primary text-primary-foreground rounded-lg p-8 shadow-lg border border-primary/30">
      <div className="flex flex-col md:flex-row items-start md:items-stretch justify-between mb-8 gap-6">
        ...
        {/* Chain icons grid for desktop, dropdown for mobile */}
        <div className="w-full md:w-1/2 flex flex-col items-end gap-4">
          <div className="block md:hidden w-full">
            ...
            <div className="mt-4 flex justify-center">
              <button
                onClick={() => handleCheckBalance(selectedChain, chains.find(c => c.id === selectedChain)?.name || '')}
                disabled={checkingChain !== null || !walletMap[selectedChain]}
                ...
                title={chains.find(c => c.id === selectedChain)?.name}
              >
                {checkingChain === selectedChain && (
          ...
          <div className="hidden md:grid w-full grid-cols-5 grid-rows-2 justify-items-end gap-3">
            {chains.map((chain) => (
              <button
                key={chain.id}
